...
const { data: users, pending: status, error: allError, refresh: refreshAll } = await useAsyncData(
    'list-users',
    async (): Promise<Rental[]> => {
        try {
            const response = await get(endpoints.asAdmin.listUsers);
            if (response.success) {
...
const columns: TableColumn<User>[] = [
            ...
            return h('div', { class: 'flex items-center gap-3' }, [
                ...
                h('div', {}, [
                    h(
                        ...
                        item.display_name?.slice(0, 15) || 'N/A'
                    ),
                    h(
                        'p',
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const color = colorMap[row.original.status] || 'default'
    ...
    <UDashboardPanel id="customers">
        ...
        <template #body>
            ...
            <UTable ref="table" v-model:column-filters="columnFilters" v-model:column-visibility="columnVisibility"
                ...
                }" class="shrink-0" :data="users" :columns="columns" :loading="status === 'pending'" :ui="{
